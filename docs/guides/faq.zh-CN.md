...
### AlphaAI 是什么？
AlphaAI 是一个 AI 驱动的加密货币交易机器人，使用大语言模型（LLM）在期货市场进行交易决策。
...
### 可以同时运行多个交易员吗？
可以！AlphaAI 支持运行多个交易员，每个可配置不同的 AI 模型和交易策略。
...
### 需要编程经验吗？
不需要！AlphaAI 有 Web 界面进行所有配置。但基础的命令行知识有助于安装和故障排查。
...
### 应该使用子账户吗？
**推荐**：是的，使用专门的子账户运行 AlphaAI 可以更好地隔离风险。但请注意，某些子账户有限制（例如币安子账户最高 5 倍杠杆）。
...
**原因**：AlphaAI 使用 `PositionSide(LONG/SHORT)`，需要双向持仓模式。

参见 [Issue #202](https://github.com/gpteth/AlphaAI/issues/202) 和[故障排查指南](TROUBLESHOOTING.zh-CN.md#-只开空单-issue-202)。
...
### AI 会从错误中学习吗？
会的，在一定程度上。AlphaAI 在每次决策提示中提供历史表现反馈，允许 AI 调整策略。
...
### 如何建议新功能？
提交 [Feature Request](https://github.com/gpteth/AlphaAI/issues/new/choose) 说明您的想法！
