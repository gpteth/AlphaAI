...
# ═══════════════════════════════════════════════════════════════
# AlphaAI AI Trading System - Docker Quick Start Script
...
read_env_vars() {
    if [ -f ".env" ]; then
        # 读取端口配置，设置默认值
        AlphaAI_FRONTEND_PORT=$(grep "^AlphaAI_FRONTEND_PORT=" .env 2>/dev/null | cut -d'=' -f2 || echo "3000")
        AlphaAI_BACKEND_PORT=$(grep "^AlphaAI_BACKEND_PORT=" .env 2>/dev/null | cut -d'=' -f2 || echo "8080")
        
        # 去除可能的引号和空格
        AlphaAI_FRONTEND_PORT=$(echo "$AlphaAI_FRONTEND_PORT" | tr -d '"'"'" | tr -d ' ')
        AlphaAI_BACKEND_PORT=$(echo "$AlphaAI_BACKEND_PORT" | tr -d '"'"'" | tr -d ' ')
        
        # 如果为空则使用默认值
        AlphaAI_FRONTEND_PORT=${AlphaAI_FRONTEND_PORT:-3000}
        AlphaAI_BACKEND_PORT=${AlphaAI_BACKEND_PORT:-8080}
    else
        # 如果.env不存在，使用默认端口
        AlphaAI_FRONTEND_PORT=3000
        AlphaAI_BACKEND_PORT=8080
    fi
}
...
# ------------------------------------------------------------------------
start() {
    print_info "正在启动 AlphaAI AI Trading System..."
    ...
    print_success "服务已启动！"
    print_info "Web 界面: http://localhost:${AlphaAI_FRONTEND_PORT}"
    print_info "API 端点: http://localhost:${AlphaAI_BACKEND_PORT}"
    print_info ""
...
status() {
    ...
    echo ""
    print_info "健康检查:"
    curl -s "http://localhost:${AlphaAI_BACKEND_PORT}/api/health" | jq '.' || echo "后端未响应"
}
...
# ------------------------------------------------------------------------
show_help() {
    echo "alphaai AI Trading System - Docker 管理脚本"
    echo ""
